...
wis.widget.Grid.prototype = {
	...
	getCurrentData : function() {
		return jQuery(this.getDomInstance()).grid('option').gridData;
	},
	getCurrentUpdateData : function() {
		return jQuery(this.getDomInstance()).grid('option').updatedRow;
	},
		...
		if (columns) {
			...
			for (var i = 0; i < columns.length; i++) {
				...
				if (column.visbleType == 'select') {
					var ref = '';
					if (refData && bindDataField in refData) {
						ref = refData[bindDataField].data;
					}
					editor.options = {
						width : 100,
						textField : "name",
