...
(function ($) {
    ...
    $.ligerDefaults.Select = {
        ...
        width: 640, //弹出选择框的宽度，默认为630 + 10     注：加10是为了后面自适应调整
        height: 510, //弹出选择框的高度
        ...
        showSearchAll: easyloader.showSearchAll ? easyloader.showSearchAll : false,//显示全站搜索按钮
        callback: {},//基教的全选函数
        resualtFunction: null,//回写的li的回调执行函数
        template: {},//模板   //gradeSearch高级搜索模板  selectedList选择组件头部模板 choseList选择组件下方模板
        shortCutAreaHeight: 80, //常用联系人选择组件的高度   add by su
        beforeOpenFunction: null//点击前执行的函数
    };
    ...
    $.ligerSelect.open = function (p, element) {
        ...
        var options = {
            ...
            width: p.width,
            height: p.height ? p.height : $.ligerDefaults.Select.height, //edit by su 常用联系人选择组件的高度差由调用者传递过来
    ...
    $.ligerSelect.submit = function (dialog, p, element) {
        ...
        if (p.maxItemCount && p.maxItemCount < count) {
            $.ligerDialog.error("您最多可以选择" + p.maxItemCount + "项");
            return;
        }
        ...
        $("li.single", dialog).each(function (i) {
            ...
            var valueStr = $("span", $(this)).text();
            if (element[0].tagName.toLowerCase() == "div") {


                if (typeof p.resualtFunction == "function") {
                    backHtml.push("<li class=\"single\" pkid=\"" + value + "\" pkname=\"" + valueStr + "\"><span>" + $("span", $(this)).text() + "</span><a class=\"t-delete-item\"></a></li>");
                } else {
        ...
        element.nextAll("input:first").val(backHiddenValue);

        (typeof p.resualtFunction == "function") && p.resualtFunction({backValue: backHiddenValue, backValueStr: backValueStr, backHiddenElement: element.nextAll("input:first")});
        dialog.close();
    ...
    $.fn.ligerDialogSelect = function (p) {
        this.each(function () {
            //add by su  增加常用联系人选择组件的高度判断
            if (!p.height && !!p.shortCutUrl) {
                p.height = $.ligerDefaults.Select.height + $.ligerDefaults.Select.shortCutAreaHeight;
            }

            p = $.extend({}, $.ligerDefaults.Select, p || {});
            //todo by yzhao 在easyloader上开一个空间存储回调函数，可能会有问题
           /* if (easyloader.dialogToTop) {
                $.ligerDialog.getWindow().document.body.selectCallBack = p.callback;
            ...
            topWindow().document.body.selectCallBack = p.callback;
            var gg = {
                initEvent: function () {
                    gg.img.click(function () {
                        if ((typeof  p.beforeOpenFunction == "function") && p.beforeOpenFunction() == false) return;
                        ...
                        var values = gg.input.nextAll("input:first").val().split(",");
                        var list = [];
                        //初始化json,用于在调用页面显示
                        for (var i = 0; i < values.length; i++) {
                            ...
                            //对不符合规则的数据 不进行获取
                            if (initObject[p.rightItemName] && (initObject[p.rightItemName] != "") && initObject[p.rightItemValue] && (initObject[p.rightItemValue] != "")) {
                                list.push(initObject);
                            }
                        }
                        p.initJson = list;
                        //打开选择对话框
                ...
                if (p.initJson) {
                    $(p.initJson).each(function (i, item) {
                        if (item != null) {
                            ...
                            if (typeof p.resualtFunction == "function") {
                                backHtml.push("<li class=\"single\" pkid=" + item[p.rightItemValue] + " pkname=" + item[p.rightItemName] + "><span>" + item[p.rightItemName] + "</span><a class=\"t-delete-item\"></a></li>");
                            } else {
                    ...
                    $(this).nextAll("input:first").val(backHiddenValue);

                    (typeof p.resualtFunction == "function") && p.resualtFunction({backValue: backHiddenValue, backValueStr: backValue, backHiddenElement: $(this).nextAll("input:first")});
        ...
        this.each(function () {
            ...
            var g = {
                initSelectedItem: function () {
                    $(p.initJson).each(function (i, item) {
                        g.addTopItem(item[p.rightItemName], item[p.rightItemValue], item);
                    });
                },
                ...
                addItem: function (i, item) {
                    ...
                    if (p.showSearchAll) {
                        $("#selectAll", ditem).before("<input name=\"searchAll\" id=\"searchAll\" type=\"checkbox\" value=\"\"/>搜全站&nbsp;")
                    }

                    if (item.query == false) {
                        $(".btn_query", ditem).css("display", "none");
                    }
                    //高级查询的选项
                    //  g.graderSearchObj = "";
                    /* if (p.iframeId != undefined && p.iframeId != "") {
                     ...
                     //参数实现由问题，需要用户把模板传进来
                     g.graderSearchObj = p.isDialog ? $("#" + p.gradeSearch, $("iframe", parent.document)[0].contentWindow.document) : $("#" + p.gradeSearch);
                     }
                    ...
                    //增加模板控制这里只是用模板 add by su
                    if (p.template.gradeSearch) {
                        $(".toolbar_query>li:first", ditem).after("<li><a href=\"javascript:void(0);\" class=\"gradeSearch ui_btn ui_btn_mini\">展开高级</a></li>");
                        $(".toolbar_query", ditem).after(p.template.gradeSearch);
                        $(".toolbar_query", ditem).next().addClass("hidden");
                    }
                ...
                initRole: function (tabId, roleUrl, roleData) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            var roleData = ["<select id='roleList' class='roleList'>"];
                            ...
                            //角色切换下拉事件
                            $("#roleList", tabContent).unbind("change").change(function () {
                                ...
                                //取得关键词，角色，树节点
                                var treeManage = $("#" + tabId).ligerGetTreeManager();
                                var keyword = $("#keyword", g.searchObj).val();
                                var treeNodeObj = treeManage.getSelected();
                                /*alert(keyword);
                                 alert(roleId);
                                 alert(treeNodeObj.data[idFieldName]);
                                 alert(rightUrl+idFieldName);*/
                                var param = "keyword=" + encodeURI(keyword);
                                if (treeNodeObj != undefined) {
                                    param += "&" + idFieldName + "=" + treeNodeObj.data[idFieldName];
                                }
                ...
                initEvents: function (url, tabId) {
                    ...
                    $(".page", g.searchObj).unbind("click").click(function () {
                        ...
                        g.loadRightData(url, {"page": page, "pkId": pkId, "keyword": keyword}, tabId);
                    });
                    ...
                    $(".r-query-button", g.searchObj).unbind("click").click(function () {
                        ...
                        if (p.showSearchAll) {
                            var searchAll = ($("#searchAll", g.searchObj).attr("checked") != undefined);
                    ...
                    $(".r-selected-li", g.searchObj).unbind("click").click(function () {
                        if (p.useDisable && $(this).attr("disabled") && $(this).attr("disabled").toLowerCase() == "disabled") return;
                        // 单选按钮的时候，当前页只允许选择一个
                        if (p.selectType.toLowerCase() == "radio") {
                    ...
                    $("#selectAll", g.searchObj).unbind("click").click(function () {
                        ...
                        var cilickFun =  topWindow().document.body.selectCallBack != undefined && topWindow().document.body.selectCallBack["selectAllClick" + intTabId];
                        /*if (easyloader.dialogToTop) {
                            cilickFun = $.ligerDialog.getWindow().document.body.selectCallBack != undefined && $.ligerDialog.getWindow().document.body.selectCallBack["selectAllClick" + intTabId];
                        } else if (easyloader.isIframe) {
                    ...
                    $("#keyword", g.searchObj).unbind("keydown.selectSearch").keydown(function (e) {
                        var key = e.which;
                        if (key == 13) {
                            var donwNode = $(this);
                            ...
                            if (p.showSearchAll) {
                                //var searchAll = ($("#searchAll",g.searchObj).attr("checked")!=undefined);
                                //param+="&searchAll="+searchAll;
                                param.searchAll = ($("#searchAll", g.searchObj).attr("checked") != undefined);
                            }
                            g.loadRightData(url, param, tabId);
                        }
                    });
                 ...
                 * @param name
                 * @param value
                 * @param option  扩展数据用于模板使用
                 */
                addTopItem: function (name, value, option) {
                    if (p.template.selectedList) {
                        $(".t-selected-ul", g.selectAreaObj).append(
                            juicer(p.template.selectedList, option)
                        );
                    }
                    else {
                ...
                selectItem: function (item) {
                    ...
                    if (!item.hasClass("selected")) {
                        ...
                        g.addTopItem(checkbox.attr("text"), checkbox.attr("id"), dataFunction.getItemByParam(dataFunction.rigthHtmlList, "id", checkbox.attr("id")));
                    }
                },
                ...
                setRightHtml: function (url, res, tabId) {
                    ...
                    if (ps && rightData.length > 0) {
                        dataFunction.rigthHtmlList = rightData.slice(0);
                        if (p.template.choseList) // 临时命名
                        {
                            // 构建默认选择
                            var idList = g.selectedIdList();
                            $.map(rightData, function (item) {
                                $.extend(item, {checked: "", selected: "", disabled: "", type: p.selectType});
                                if ($.inArray(item.id + "", idList) >= 0) {
                                    item.checked = "checked";
                                    item.selected = "selected";
                                }
                                if (p.useDisable && item[p.rightItemDisable]) {
                                    item.disabled = "disabled";
                                }
                            });
                            rightContentHtml.push(juicer(p.template.choseList, {items: rightData}));
                        }
                        else {
                ...
                clearHtml: function () {
                    ...
                    $("#searchAll", g.select).removeAttr("checked");
                },
                ...
                autoComplete: function (tabId) {
                    //绑定keyword的自动完成事件,必须查找每个tab的keyword进行绑定
                    if (p.autoComplete) {
                        //var tabId = $("li.l-selected", ".l-tab-links>ul").attr("tabid");
                        var tabContent = $("div[tabid=" + tabId + "]", ".l-tab-content");
                        $(".autoComplete", tabContent).autocomplete({
                            source: function (request, response) {
                                $.ajax({
                                    url: p.autoComplete.url,
                                    dataType: "json",
                                    data: {
                                        pagesize: p.autoComplete.pagesize ? p.autoComplete.pagesize : 10,
                                        keyword: request.term
                                    },
                                    success: function (data) {
                                        var tempStr = p.autoComplete.splitChar ? p.autoComplete.splitChar : ",";
                                        response($.map(data.Rows, function (item) {
                                            return {
                                                label: item[p.autoComplete.pkName] + (item[p.autoComplete.showName] ? tempStr + item[p.autoComplete.showName] : ""),
                                                value: item[p.autoComplete.pkName]
                                            }
                                        }));
                                    }
                                });
                ...
                loadRightData: function (url, param, tabId, defaultRoleId) {
                    //把用户选中的角色传递到后台
                    var tabContent = $("div[tabid=" + tabId + "]");
                    var roleId = defaultRoleId ? defaultRoleId : $("#roleList", tabContent).find("option:selected").val();
                    g.clearHtml();
                    //g.loading.show();
                    ...
                    if (param != null && roleUrl != undefined && roleId != undefined && roleUrl != "") {
                        //param += "&roleId=" + roleId;
                        param.roleId = roleId;
                    }
                ...
                initShortCut: function () {
                    $(".shortCut", "#shortCutArea").unbind("click").click(function () {
                        ...
                        if ($(this).hasClass("li_select")) {
                            ...
                            $(this).addClass("li_select");
                            g.addTopItem(name, value, dataFunction.getItemByParam(dataFunction.rigthHtmlList, "id", value));
                            /* $(".t-selected-ul", g.selectAreaObj).append('<li id="t-selected-item-' + value + '" class="single"><span>' + name + '</span><a class="t-delete-item"></a></li>');*/
                ...
                backSpaceDown: function () {
                    //绑定本tab的页面上的id为keyword的回车事件,必须先解除绑定，否则会提交多次
                    $(document).unbind("keydown.deleteSelect").keydown(function (e) {
                        var key = e.which;
                        var keyEvent = true;
                        if (key == 8) {
                            var elementObj = e.srcElement || e.target;
                            if (elementObj.tagName.toLocaleLowerCase() == "input" || elementObj.tagName.toLocaleLowerCase() == "textarea") {
                                keyEvent = elementObj.disabled || elementObj.readOnly;
                            }
                        } else {
                            keyEvent = false;
                        }
                        ...
                        if (keyEvent) {
                            //模拟x号的点击删除动作,并且事件要先解除绑定
                            $(".t-selected-ul>li:last>a.t-delete-item", g.selectAreaObj).click();
                        }

                        //监听到事件触发默认的动作
                        keyEvent && e.preventDefault();
                    });
                },
                selectedIdList: function () {
                    var idList = [];
                    $("li.single", g.select).each(function () {
                        idList.push($(this).attr("id").substr("t-selected-item-".length));
                    });
                    return idList;
                }
            ...
            /**
             *  缓存
             * @type {{rigthHtmlList: Array, getItemByParam: Function}}
             */
            var dataFunction = {
                /**
                 * 缓存右侧当前页的数据原型
                ...
                rigthHtmlList: [],
                /**
                 * 通过匹配规则 和匹配值 从 列表中 返回指定的 内容对象（唯一）
                 * @param list
                 * @param paramString
                 * @param pkValue
                 * @returns {{}}
                 */
                getItemByParam: function (list, paramString, pkValue) {
                    var item = {};
                    $.each(list, function (index, m) {
                        if (m[paramString + ""] == pkValue) item = m;
                    });
                    return item;
                }
            };
            ...
            //if (p.simpleSelect) $(frame[1]).addClass("hidden");
            ...
            if (p.shortCutUrl != "") {
                //edit by su 将常用联系人选择组件的高度提取出来
                g.selectAreaObj.after('<div id="shortCutArea" style="height:' + p.shortCutAreaHeight + 'px;"></div>');
            ...
            $("#content", g.select).ligerTab({
                onAfterSelectTabItem: function (tabId) {
                    ...
                    var roleId = li.attr("roleId") == undefined ? "" : li.attr("roleId");//""全部人员
                    //兼容原来的默认不指定取pkId,treeNodeName
                    var idFieldName = li.attr("idFieldName") == undefined ? "pkId" : li.attr("idFieldName");
                    ...
                    if (leftUrl) {
                        tabItem.ligerTree({
                            ...
                            onSelect: function (node) {
                                ...
                                var params = {"nodeData": JSON.stringify(node.data)};
                                params[idFieldName] = node.data[idFieldName];

                                g.loadRightData(rightUrl, params, tabId, roleId);
                                g.treeNodeClick(tabId);
                                g.nodeData = params;

                                //角色权限初始化
                                g.initRole(tabId, roleUrl, params);
                            },
