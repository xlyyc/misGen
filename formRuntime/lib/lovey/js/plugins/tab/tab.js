...
(function ($) {
    ...
    $.ligerDefaults.Tab = {
        ...
        favTaskBar: "菜单收藏到任务栏",
        unFavTaskBar: "取消收藏",
        showTaskBarMenu:true,//显示收藏到任务栏的菜单
    ...
    $.fn.ligerTab = function (p) {
        ...
        this.each(function () {
                        ...
                        if (url != undefined && url != '') {
                            ...
                            setTimeout(function () {
                                ...
                                if (options.resizeFrame) {
                                    ...
                                    topWindow().setFrameZoneHeight(topWindow().top_expanded);    
                                }
                            }, 0);
                ...
                addTabItem: function (options) {
                    ...
                    if (url != undefined && url != '') {
                        ...
                        setTimeout(function () {
                            ...
                            //by yzhao 当抽屉组件和tab组件结合使用，需要刷新高度
                            if (options.resizeFrame) {
                                 self.setFrameZoneHeight &&  self.setFrameZoneHeight((!! self.top_expanded ?  self.top_expanded : false));
                            }
                        }, 0);
                    ...
                    if(p.tabTitleWidth)
                    {
                        easyloader.debug("is ie6 >>>" + easyloader.isIE6);
                        easyloader.debug("$('a', tabitem).width() >>>" + $("a", tabitem).width());
                        easyloader.debug("tabTitleWidth >>>" + p.tabTitleWidth);
                        if(easyloader.isIE6)
                        {
                            ( $("a", tabitem).width() > p.tabTitleWidth ) && $("a", tabitem).width(p.tabTitleWidth);
                        }
                        else
                        {
                            $("a", tabitem).css("max-width",p.tabTitleWidth);
                        }
                    }
                ...
                menuItemClick: function (item) {
                    ...
                    switch (item.id) {
                        ...
                        case "fav":
                            ...
                            {
                                $.ligerDialog.getWindow().taskbar.addTaskBar({
                        ...
                        case "unfav":
                           /* if(easyloader.dialogToTop)
                            {
                                $.ligerDialog.getWindow().taskbar.removeTaskBar(g.actionTabid);
                            }
                            else if (easyloader.isIframe) {
                                //因为整个平台是可以被Iframe嵌入的，需要保证tab时可用的
                                parent.taskbar.removeTaskBar(g.actionTabid);
                            } else {
                                self.taskbar.removeTaskBar(g.actionTabid);
                            }*/
                            topWindow().taskbar.removeTaskBar(g.actionTabid);
                            g.actionTabid = null;
                            break;
                    }
                },
            ...
            if (p.contextmenu && $.ligerMenu) {
                ...
                p.contextmenu && p.showTaskBarMenu && itemsObj.push({text: p.favTaskBar, id: 'fav', click: po.menuItemClick});
                p.contextmenu && p.showTaskBarMenu && itemsObj.push({text: p.unFavTaskBar, id: 'unfav', click: po.menuItemClick});

                g.tab.menu = $.ligerMenu({width: 150, items: itemsObj });
            }
            ...
            $("> div", g.tab.content).each(function (i, box) {
               ...
               /* if(p.tabTitleWidth)
                {
                    $(".maintab_left > a", li).css("max-width",p.tabTitleWidth);
                   // $(".maintab_left > a", li).width(p.tabTitleWidth);
                }*/
                if ($(box).attr("title")) {
