...
(function ($) {
    ...
    $.loveyDefaults.taskBar = {
        ...
        withTab: true//和tab组合使用
    };

    $.loveyDefaults.taskBarString = {
    ...
    $.fn.loveyTaskBar = function (options) {
        var op = $.extend({}, $.loveyDefaults.taskBar, options || {});
...
//                        {
//                            $.ligerDialog.getWindow().navtab.addTabItem({
//                                text: title ? title : '空菜单',
//                                url: url ? url : '#',
//                                tabid: tabid,
//                                resizeFrame: true,
...
//                        }
                ...
                $("li >div", g.taskBar).unbind("click").click(function () {
                    g.removeTaskBar($(this).attr("taskid"), function () {
                        if (!op.withTab) return false;
                        //刷新 搜藏图标/按钮
                        var collectObj = "";
                        if(easyloader.dialogToTop)
                        {
                            collectObj = $(".collect:last", $("iframe:first", "div[tabid=" + $.ligerDialog.getWindow().navtab.getSelectedTabItemID() + "]")[0].contentWindow.document);
                        }
                        else if (easyloader.isIframe) {
                            collectObj = $(".collect:last", $("iframe:first", "div[tabid=" + parent.navtab.getSelectedTabItemID() + "]")[0].contentWindow.document);
                        } else {
                            collectObj = $(".collect:last", $("iframe:first", "div[tabid=" + self.navtab.getSelectedTabItemID() + "]")[0].contentWindow.document);
                        }
