...
(function ($) {
    ...
    $.ligerDefaults.Tree = {
        ...
        idFieldName: 'pkId',
        iconFieldName: 'icon',//个性化ICON标识字段
        classNameFieldName: 'className',//个性化样式标识字段
        clickSpanWithAction: easyloader.clickSpanWithAction ? easyloader.clickSpanWithAction : false,//点击节点名是否可以进行展开收起操作
        ...
        slide: false, //是否以动画的形式显示,树启用菜单的时候定位和宽度获取不能用
    ...
    $.fn.ligerTree = function (p) {
        ...
        this.each(function () {
            ...
            var g = {
                ...
                /**
                 * 多选节点 不对节点进行反操作
                 * @param selectNodeParm
                 */
                selectNode_Mul: function (selectNodeParm) {
                    var clause = null;
                    if (typeof (selectNodeParm) == "function") {
                        clause = selectNodeParm;
                    } else if (typeof (selectNodeParm) == "object") {
            ...
            var po = {
                ...
                loading:function(status)
                {
                    if(status) 
                    {
                        g.loading.addClass("l-tree-loading");    
                    }
                    else
                    {
                        g.loading.removeClass("l-tree-loading");
                    }
                    g.loading.show()
                },
                //添加项到g.data
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            if (g.hasChildren(data[i])) {
                                ...
                                if (p.parentIcon) {
                                    ...
                                    treehtmlarr.push(po.getParentNodeClassName(data[i], isExpandCurrent) + " ");
                                ...
                                if (p.childIcon) {
                                    ...
                                    treehtmlarr.push(po.getChildNodeClassName(data[i]) + " ");
                                    if (p.iconFieldName && o[p.iconFieldName])
                                        treehtmlarr.push('l-tree-icon-none');
                                    treehtmlarr.push('">');
                                    if (p.iconFieldName && o[p.iconFieldName])
                            ...
                            //显示菜单名称
                            var className = p.classNameFieldName && o[p.classNameFieldName] && o[p.classNameFieldName];
                            if (data[i][p.disableFlag] == false) {
                                treehtmlarr.push('<span class="gray '+className+'">' + data[i][p.textFieldName] + '</span></div>');
                            } else {
                                treehtmlarr.push('<span class="'+className+'">' + data[i][p.textFieldName] + '</span></div>');
                            }
                            if (g.hasChildren(data[i])) {
                                var isLastNew = [];
                ...
                setTreeEven: function () {
                    ...
                    g.tree.click(function (e) {
                        ...
                        if (p.clickSpanWithAction && folderitem.length == 0) {
                            folderitem = $(obj).prev();//如果点击节点也可以收放，则获取前辈元素
                        }
                        var treedataindex = parseInt(treeitem.attr("treedataindex"));
                        var treenodedata = po.getDataNodeByTreeDataIndex(g.data, treedataindex);
                        ...
                        if ($(obj).hasClass("l-checkbox") && !$(obj).siblings().hasClass("gray")) {
                        ...
                        } else if (treeitembtn.hasClass("l-expandable-open") && (clickOnTreeItemBtn || p.clickSpanWithAction)) {
                            //状态：已经张开
                            ...
                            if (!(obj.tagName.toLowerCase() == 'span') || p.clickSpanWithAction) {
                                treeitembtn.removeClass("l-expandable-open").addClass("l-expandable-close");
                                //单选多选操作的元素不一致，需要注意
                                if (p.checkbox) {
                                    folderitem.next().removeClass("l-tree-icon-" + p.parentIcon + "-open").addClass("l-tree-icon-" + p.parentIcon);
                                } else {
                                    folderitem.removeClass("l-tree-icon-" + p.parentIcon + "-open").addClass("l-tree-icon-" + p.parentIcon);
                                }
                                if (p.slide)
                        ...
                        } else if (treeitembtn.hasClass("l-expandable-close") && (clickOnTreeItemBtn || p.clickSpanWithAction)) {
                            //状态：没有张开
                            if (p.onBeforeExpand && p.onBeforeExpand({ data: treenodedata, target: treeitem[0] }) == false)
                                return false;

                            var callback = function () {
                                p.onExpand && p.onExpand({ data: treenodedata, target: treeitem[0] ,tree:g });
                            };
                            ...
                            if (!(obj.tagName.toLowerCase() == 'span') || p.clickSpanWithAction) {
                                ...
                                if (p.checkbox) {
                                    folderitem.next().removeClass("l-tree-icon-" + p.parentIcon).addClass("l-tree-icon-" + p.parentIcon + "-open");
                                } else {
                                    folderitem.removeClass("l-tree-icon-" + p.parentIcon).addClass("l-tree-icon-" + p.parentIcon + "-open");
                                }
