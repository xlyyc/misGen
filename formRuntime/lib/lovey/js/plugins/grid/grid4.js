            ...
            toggleCol: function (columnparm, visible) {
                ...
                if (p.doServerUrl != "") {
                    ...
                    $.ajax({
                        type: "post",
                        url: p.doServerUrl,
                        data: {"columnDate": JSON.stringify(columnObject)},
                        dataType: "json",
                        success: function (data) {
                            if (data.status) {
                                if (visible) {
        ...
        var po = {
            ...
            initBuildGridHeader: function () {
                ...
                if(p.rownumbers){
                    ...
                    rownumbersHeaderCell.css({ width: 26,height:26});
                    $("tr:first", g.gridheader1).append(rownumbersHeaderCell);
                    g.headers.push({
                        width: 26,
                        isrownumbers: true
                    });
            ...
            initEven: function () {
                ...
                if ($.fn.ligerDrag && p.colDraggable) {
                    ...
                    $(".l-grid-hd-cell-inner", g.gridheader).ligerDrag(
                        {
                            proxyX: 0,
                            proxyY: 0,
                            proxy: function (draggable, e) {
                                //构造一个代理层
                                var content = $(".l-grid-hd-cell-text:first", e.currentTarget).text();
                ...
                if ($.fn.ligerDrag && p.rowDraggable) {
                    ...
                    $(".l-grid-row", g.gridbody).ligerDrag({
                        proxyX: 0,
                        proxyY: 0,
                        proxy: function (draggable, e) {
                            //构造一个代理层
                            var content = $(".l-grid-hd-cell-text:first", e.currentTarget).text();
                            g.proxy = $("<div class='l-drag-proxy' style='display:none'><div class='l-drop-icon l-drop-no'></div></div>").appendTo('body');
                            g.proxy.append(content);
                            return g.proxy;
                        },
                        onStartDrag: function (e) {
                            g.draggingRows = g.getRow(e.target.attr("rowid"));
                            g.validRange = {
                        ...
                        onDrag: function (e, event) {
                             ...
                             * @type {*}
                             */
                            var rowdata = g.draggingRows;
                            if (!rowdata) return false;
                            if (g.rowDropIn == null) g.rowDropIn = -1;
                            var rows = g.draggingRows ? g.draggingRows : [rowdata];
                            var pageX = event.pageX;
                            var pageY = event.pageY;
                            var visit = false;
                            var validRange = g.validRange;
                            if (pageX < validRange.left || pageX > validRange.right
                                || pageY > validRange.bottom || pageY < validRange.top) {
                                g.rowDropIn = -1;
                                g.rowDroptip.hide();
                                g.proxy.find(".l-drop-icon:first").removeClass("l-drop-yes l-drop-add").addClass("l-drop-no");
                                return;
                            }
                            g.rows = g.currentData[p.root];
                            for (var i in g.rows) {
                                var rd = g.rows[i];
                                var rowobj = g.getRowObj(i);
                                var rowid = $(rowobj).attr("rowid");
                                ...
                                if (pageX > range.left && pageX < range.right && pageY > range.top && pageY < range.bottom) {
                                    ...
                                    if (p.tree && pageY > range.top + 5 && pageY < range.bottom - 5) {
                                        g.proxy.find(".l-drop-icon:first").removeClass("l-drop-no l-drop-yes").addClass("l-drop-add");
                                        g.rowDroptip.hide();
                                        g.rowDropInParent = true;
                                    }
                                    else {
                                        g.proxy.find(".l-drop-icon:first").removeClass("l-drop-no l-drop-add").addClass("l-drop-yes");
                                        g.rowDroptip.show();
                        ...
                        onStopDrag: function (e, event) {
                            var rows = [g.draggingRows];
                            g.rowdragging = false;
                            ...
                            if (g.rowDropIn != -1) {
                                if (p.tree) {
                                    ...
                                    for (var j in rows)
                                    {
                                         ...
                                         * 拖动行号，目标行号
                                         */
                                        var fromRow = g.getRow(rows[j]);
                                        var toRow = g.getRow(g.rowDropIn);
                                        ...
                                        listdata.splice(toIndex + (g.rowDropDir == "bottom" ? 1 : 0), 0, fromRow);

                                        //重新刷新数据
                                        var loadData={};
                                        loadData.Rows=listdata;
                                        loadData.Total=p.data[p.record];
                                        po.showData(loadData);
                                        //重新绑定事件
                                        //po.initEven();
                                    }
                                }
            ...
            getHtmlFromData: function (dataArray, tree, level, parentrowid) {
                ...
                $(dataArray).each(function (i, item) {
                    ...
                    $(g.headers).each(function (headerCellIndex, headerInfor) {
                        ...
                        //如果是行序号
                        else if(this.isrownumbers){
                            var rowcell = '<tr><td class="l-grid-row-cell l-grid-row-cell-rownumbers" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner l-grid-row-cell-inner-fixedheight">1</div></td></tr>';
