    ...
    $.ligerMenu = function (p) {
        ...
        var g = {
            show: function (options, menu) {
                /**
                 * @update：修改top、left定位为原版，如导致其他问题请更新到上一个版本
                 * @author：宋士杰
                 * @date：2013-07-08
                 * */
                ...
                var singleRowObj = $(".l-menu-item[singlerow=true]");

                if ($(".l-checked").size() > 1) {
                    singleRowObj.addClass("l-menu-item-disable");
                } else {
                    (singleRowObj.size() > 1) && $(".l-menu-item").removeClass("l-menu-item-disable");
                }
                menu.show();
                g.updateShadow(menu);
            },
