    ...
    $.fn.ligerAccordion = function (p) {
        this.each(function () {
            p = $.extend({
                ...
                height: null,
                speed: "fast", //动画延时,已废弃在低级浏览器里面元素多了后会有严重性能问题
                ...
                accordRootClickFunction: null,//二级菜单的点击执行函数
            ...
            var g = {
                ...
                addTab: function (options) {
                    //由于 accordion 和 tab 始终在同一个dom对象内，所以不需要分dom获取对象
...
//                    }
            ...
            $("> div", g.accordion).each(function (i, box) {
                var header = $('<div class="l-accordion-header" index="' + i + '"><div class="l-accordion-toggle"></div><div class="l-accordion-header-inner"></div></div>');
                ...
                if (!$(box).hasClass("scrollArea")) $(box).addClass("scrollArea");
            });

            //添加tab
            $("> .l-accordion-content li a", g.accordion).bind("click", function () {
                if (typeof p.accordRootClickFunction == "function") {
                    p.accordRootClickFunction($(this));
                    return;
                }
                ...
                function () {
                    ...
                    if (aElement.hasClass(p.childClassName)) {
                        ...
                        if (dialogElement.html() != null) {
                            autoFload(easyloader.topExpanded);
                            dialogElement.show();
                        } else {
            ...
            $(">.l-accordion-header", g.accordion).click(function () {
                if (!$(this).hasClass("l-accordion-selected")) {
                    ...
                    $(this).removeClass("l-accordion-selected");
                }

                var imageObj = $(this).find("img");
                var srcUrl = imageObj.attr("src");
                var tSrcUrl = imageObj.attr("t_src");
                imageObj.attr("src", tSrcUrl);
                imageObj.attr("t_src", srcUrl);

                //取得点击元素的兄弟节点，有且一个。
                var britherObj = $(this).siblings(".l-accordion-selected");
                britherObj.removeClass("l-accordion-selected");
                britherObj.each(function (i, item) {
                    var tempObj = $(item).find("img");
                    var srcUrl = tempObj.attr("src");
                    var tSrcUrl = tempObj.attr("t_src");
                    tempObj.attr("src", tSrcUrl);
                    tempObj.attr("t_src", srcUrl);
                });
                ...
                if (togglebtn.hasClass("l-accordion-toggle-close")) {
                    ...
                    //IE底下有BUG暂不启用,当节点多的时候严重影响性能
                    //$(this).next(".l-accordion-content").show(p.speed).siblings(".l-accordion-content:visible").hide(p.speed);
                    $(this).next(".l-accordion-content").css({"display": "block"}).siblings(".l-accordion-content:visible").css({"display": "none"});
                    ...
                    $(this).next(".l-accordion-content").css({"display": "none"});
                }
            });
            //init
