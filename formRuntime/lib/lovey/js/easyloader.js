...
(function ($) {
    ...
    easyloader = {
        ...
        timeout: 2000, //加载超时事件
        isIE6:(navigator.appName == "Microsoft Internet Explorer" && parseInt(navigator.appVersion) == 4 && navigator.appVersion.indexOf("MSIE 6.0") != -1),//判断浏览器是否是ie6
        logLevel:0,
        printFunction:null,
        debug:function(obj){
            if( this.logLevel == 4 )
            {
                window.console && console.log("[DEBUG]:"+ obj)
            }
        },
    ...
    function loadJs(url, callback) {
        ...
        script.onload = script.onreadystatechange = function () { //onload是firefox 浏览器事件，onreadystatechange,是ie的，为了兼容，两个都写上，这样写会导致内存泄露
            ...
            if (!done && (!script.readyState || script.readyState == 'loaded' || script.readyState == 'complete')) {
                ...
                if (callback) {//加载后执行回调
                    easyloader.printFunction && easyloader.printFunction(url,"js");
    ...
    function loadCss(url, callback) {
        ...
        if (callback) {
            easyloader.printFunction && easyloader.printFunction(url,"css");
            callback.call(link);
        }
    }
