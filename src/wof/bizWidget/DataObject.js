...
wof.bizWidget.DataObject.prototype = {
    ...
    _queryPolicy:null, //二次过滤策略(默认本地二次过滤)   本地 local  远程 remote
    ...
    /**
     * 过滤主缓冲区 存放从主缓冲区中过滤后的引用(以构件ID作为key)
     *
    ...
    /**
     * 过滤删除缓冲区 存放从对应过滤主缓冲区删除的引用(以构件ID作为key)
     *
     */
    ...
    _new: function(componentId, entityAlias, entityData){
        ...
        //在主缓冲区中增加该条数据(需要保证顺序 并同时在对应的过滤缓冲区增加该数据的引用)
        //同时该数据在主缓冲区的状态为New或者NewModified(要结合实体的元数据中的默认值来决定是何种状态)
        //并且在对应的过滤缓冲区加入该数据引用
     ...
     * entityDataId 实体数据id
     */
    _delete: function(componentId, entityAlias, entityDataId){
     ...
     * pageNo 页号(从1开始)
     */
    _query: function(entityAlias, queryParam, pageSize, pageNo){
     ...
     * entityData 实体数据
     */
    _saveOrUpdate: function(entityData){
        var entityId = this.getEntityId();
     ...
     * entityAlias 实体别名
     */
    _reset: function(entityAlias){
        /**
         * 从原始缓冲区恢复数据到主缓冲区和删除缓冲区
         * 并将主缓冲区的状态和删除缓冲区的行列状态恢复为默认值(NotModified)
         * 注意如果是是新行(New和NewModified) 则忽略此数据不做恢复
         *
         * 如果 实体别名 为空
         * 则恢复全部数据
         * 否则只恢复指定 实体别名 对应的数据
     ...
     * 回收内存
     *
     * 此内存清理方法会移除原始缓冲区(包括主缓冲区)所有没有在过滤缓冲区和删除缓冲区引用的数据
     */
    _gc: function(){
        //如果原始缓冲区(包括主缓冲区)数据超过设定数量 则执行清理方法
        if(this._dataTotal>this.getTotalDataNumber()){
            console.log('开始回收原始、主缓冲区中的多余数据');
        }
    },
