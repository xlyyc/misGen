/**
 * @bizWidgetClass SearchComponent class
...
wof.bizWidget.SearchComponent.prototype = {
    ...
    _callStr: null,    //调用字符串，格式为 构件类型唯一识别串:构件版本，例如【SearchComponent:1.0.0】
    ...
    beforeRender: function () {
        if(this._initFlag==null){
            ...
            //todo 需要删除
            this.getDomInstance().append(jQuery('<span style="position:absolute;top:20px;left:20px;">搜索 待实现</span>'));
        }
    },
    ...
    createSelf: function(width, height){
        var node = new wof.bizWidget.SearchComponent();
        node.setWidth(width);
