...
wof.functionWidget.spanner.UpdateRecordComponentSpanner = function () {
    ...
    onReceiveMessage.push({id:'wof.bizWidget.Spanner_render',method:'var propertys=message.sender.propertys;if(propertys.className=="wof.functionWidget.UpdateRecordComponent"){this.setPropertys(propertys);}else{this.setPropertys(null)}this.render();'});
    var method = 'var data=message.sender.propertys; '
        +'if(data.id==this.getPropertys().id){ '
        +' var node=wof.util.ObjectManager.get(data.id); '
        +' node.updateUpdateRecordComponent(data); '
        +' node.render();'
        +'}';
...
wof.functionWidget.spanner.UpdateRecordComponentSpanner.prototype = {
    ...
    afterRender: function () {
        ...
        if(updateRecordComponent!=null){
            ...
            activeData.activeClass = 'UpdateRecordComponent';
    ...
    exportData: function(node){
        ...
        if(node.getClassName()=='wof.functionWidget.UpdateRecordComponent'){
            ...
            paramMap2.compParamValue = node.getFormFunctionId().formFunctionId;
